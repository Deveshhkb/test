...
$(document).ready(function () {
  ...
  // let API_TOKEN =
  //   "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJzZWphbDM3NzAiLCJ1c2VydHlwZSI6IjMiLCJleHAiOjE2OTMzMDI0NTYsImlhdCI6MTY5MzIxNjA1Nn0.afL6XdZXdIKGiQkR3clEfGHooXT1b8dXyJpO1mEmoyo";
  ...
  function getMyBetData() {
    axios
      ...
      .then((res) => {
        $("#mybetdetalis").empty();
        const mybetdata = res.data.data["VDragon Tiger"];
  ...
  try {
    function getLast10GameResult(timeout = 0) {
      axios.get(API_URL + "CasinoAdmin/GetData/dt20Result").then((res) => {
        ...
        if (winnermid === mid && !resultShown) {
          ...
          setTimeout(() => {
            ...
            $(".liabilityData_2").html("");
            $(".liabilityData").html("");
            $(".liabilityData_1").html("");
          }, 1500);
          $(".removepchbg").empty();
  ...
  function betkeyrate(t2) {
    ...
    for (let t = 0; t < rowArr.length; t++) {
      ...
      if (t < 12) {
        $("#betKeyAction_" + (1 + t)).html(dataObj[data].rate);
      } else {
        $("#dargonrate").html(dataObj["dragon card 1"].rate);
        $("#tigerrate").html(dataObj["tiger card 1"].rate);
      }
    }
  }
  ...
  async function getgameDatat1() {
    let apires;
    try {
      apires = await axios.get(API_URL + "CasinoAdmin/GetData/dt20Data");
  ...
  function randomChipPlaceHelper(tableId) {
    ...
    $("<div/>", {
      ...
      style: `background-image: url(/games/dragonTiger_Web/assets/images/${
  ...
  function closebetsound() {
    var audio = $("<audio>");
    audio.attr("src", "/games/dragonTiger_Web/assets/audio/stopbetting.mp3");
  ...
  function openbetsound() {
    var audio = $("<audio>");
    audio.attr("src", "/games/dragonTiger_Web/assets/audio/startbetting.mp3");
    audio.on("ended", removebg);
    if (!isMuted) {
      audio[0].play().catch(() => {});
    }
  ...
  function coinsSound() {
    var audio = $("<audio>");
    audio.attr("src", "/games/dragonTiger_Web/assets/audio/coinsound.wav");
    audio.on("ended", removebg);
    if (!isMuted) {
      audio[0].play().catch(() => {});
    }
  ...
  function cardflip() {
    var audio = $("<audio>");
    audio.attr("src", "/games/dragonTiger_Web/assets/audio/flipcard.mp3");
    audio.on("ended", removebg);
    if (!isMuted) {
      audio[0].play().catch(() => {});
    }
  ...
  function wincupsound() {
    var audio = $("<audio>");
    audio.attr("src", "/games/dragonTiger_Web/assets/audio/winsong.mp3");
    audio.on("ended", removebg);
    if (!isMuted) {
      audio[0].play().catch(() => {});
    }
  ...
  objPlayPauseMusic.src = "/games/dragonTiger_Web/assets/audio/bgm.mp3";
