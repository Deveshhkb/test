...
$(document).ready(function () {
  ...
  // let API_TOKEN =
  //   "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJzZWphbDM3NzAiLCJ1c2VydHlwZSI6IjMiLCJleHAiOjE2OTMzMDk0OTIsImlhdCI6MTY5MzIyMzA5Mn0.vfWl0yS8LMC4S2SdvUOR42HIo4HF7-XZ45Om_wUJcyg";
  ...
  function getMyBetData() {
    axios
      ...
      .then((res) => {
        const mybetdata = res.data.data["VAmar Akbar Anthony"];
        $("#mybetdetalis").empty();
  ...
  try {
    function getLast10GameResult(timeout = 0) {
      axios.get(API_URL + "CasinoAdmin/GetData/aaaResult").then((res) => {
        ...
        if (winnermid === mid && !resultShown) {
          ...
          const result = lastGames[0].winner;
          if (result == 1) {
            $("#currentResultName").html("Amar");
          } else if (result == 2) {
            $("#currentResultName").html("Akbar");
          } else {
            $("#currentResultName").html("Anthony");
          }
  ...
  function popupurl(urllink) {
    try {
      axios.get(urllink).then((res) => {
        ...
        let dResult = resultDetail[0];
        let tResult = resultDetail[1];
        let lResult = resultDetail[2];
        let wResult = resultDetail[3];
        const result = data.winner;
        if (result == 1) {
          $("#resultName").html("Amar");
        } else if (result == 2) {
          $("#resultName").html("Akbar");
        } else {
          $("#resultName").html("Anthony");
        }

        $("#hisDatatxt_0").html(`${dResult}`);
        $("#hisDatatxt_1").html(`${tResult}`);
        $("#hisDatatxt_2").html(`${lResult}`);
        $("#hisDatatxt_3").html(`${wResult}`);
  ...
  function betkeyrate(t2) {
    ...
    for (let t = 0; t < rowArr.length; t++) {
      ...
      if (t <= 2) {
        ...
        liveOddsDataObj[4 + t] = dataObj[data];
        $("#betKeyAction_" + (4 + t)).html(dataObj[data].rate);
      } else if (t <= 21) {
        liveOddsDataObj[4 + t] = dataObj[data];
      }
    }
    $("#cardRate").html(`Cards ${dataObj["card 2"].rate}`);
  }
  ...
  async function getgameDatat1() {
    let apires;
    try {
      apires = await axios.get(API_URL + "CasinoAdmin/GetData/aaaData");
      ...
      t1.forEach(function (data, i) {
        ...
        if (gameStatust2 == 0 && timeLeftGame == 0 && hidden) {
          window.location.hash = "#moneybutton";
          $(".settingprnt").hide();
          $("#gamestart").removeClass("betStart");
  ...
  function randomChipPlaceHelper(tableId) {
    ...
    $("<div/>", {
      ...
      style: `background-image: url(/games/aaa_mobile/assets/images/${
  ...
  $("#betplacebtn").bind("click", function () {
    if (confirmBet < 26 && isbet) {
      ...
      if (confirmBet % 2 == 0 && confirmBet < 7) {
        isBackAct = false;
        dataRate = "layrate";
      } else {
        isBackAct = true;
      ...
      betPlaceApiCall(
        API_TOKEN,
        {
          casinoName: 1,
          isBack: isBackAct,
          odds: liveOddsDataObj[confirmBet][dataRate],
          stake: amount,
  ...
  function closebetsound() {
    var audio = $("<audio>");
    audio.attr("src", "/games/aaa_mobile/assets/audio/stopbetting.mp3");
  ...
  function openbetsound() {
    var audio = $("<audio>");
    audio.attr("src", "/games/aaa_mobile/assets/audio/startbetting.mp3");
    audio.on("ended", removebg);
    if (!isMuted) {
      audio[0].play().catch(() => {});
    }
  ...
  function coinsSound() {
    var audio = $("<audio>");
    audio.attr("src", "/games/aaa_mobile/assets/audio/coinsound.wav");
    audio.on("ended", removebg);
    if (!isMuted) {
      audio[0].play().catch(() => {});
    }
  ...
  function cardflip() {
    var audio = $("<audio>");
    audio.attr("src", "/games/aaa_mobile/assets/audio/flipcard.mp3");
    audio.on("ended", removebg);
    if (!isMuted) {
      audio[0].play().catch(() => {});
    }
  ...
  function wincupsound() {
    var audio = $("<audio>");
    audio.attr("src", "/games/aaa_mobile/assets/audio/winsong.mp3");
    audio.on("ended", removebg);
    if (!isMuted) {
      audio[0].play().catch(() => {});
    }
  ...
  objPlayPauseMusic.src = "/games/aaa_mobile/assets/audio/bgm.mp3";
