...
/**
 * In-memory game server. Mirrors what a real RGS (remote game server) would
 * do: it owns the balance, the RNG, feature state and win evaluation. The
 * client only animates what this returns, so swapping in a real backend is a
 * transport change, not a game change.
 */
...
export const mockServer = new MockGameServer();
